...
use fungi_transport::{ChannelBuilder, Duplex, Pseudonymous, RecvChannel, SendChannel};
use std::io;
...
type Fixture = Duplex<Sender, Receiver>;

fn pair(max: usize) -> (Fixture, Fixture) {
    let (left, incoming_right) = mpsc::channel(1);
    let (right, incoming_left) = mpsc::channel(1);
    (
        Duplex::new(Sender { queue: left, max }, Receiver(incoming_left)),
        Duplex::new(Sender { queue: right, max }, Receiver(incoming_right)),
    )
}
...
fn faulty(channel: Fixture, fault: Fault) -> Duplex<Sender, FaultyReceiver> {

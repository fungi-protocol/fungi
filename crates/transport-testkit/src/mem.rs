...
use fungi_transport::{
    ChannelBuilder, Duplex, PeerChannel, Pseudonymous, RecvChannel, SendChannel,
};
use tokio::sync::mpsc;
...
/// Identity of one in-memory endpoint.
#[derive(Debug, Clone)]
...
pub type MemChannel<M = Vec<u8>> = Duplex<MemSender<M>, MemReceiver<M>>;
...
impl<M: Send> Mailbox<M> {
    ...
    pub fn connect(&mut self) -> Duplex<MemSender<M>, MailboxReceiver<'_, M>> {
        Duplex::new(self.sender.clone(), MailboxReceiver(&mut self.receiver))
    }
}
...
mod tests {
    ...
    #[test]
    fn directions_preserve_peer_identity() {
        let (first, _) = duplex::<Message>(MemConfig::default());
        let (second, _) = duplex::<Message>(MemConfig::default());
        fn pseudonymous<S: SendChannel<Message, Privacy = Pseudonymous>>(_: &S) {}
        pseudonymous(&first);
        let (sender, receiver) = first.into_parts();
        let (other, _) = second.into_parts();
        assert_eq!(sender.peer(), receiver.peer());
        assert_ne!(sender.peer(), other.peer());
    }
    ...
    async fn typed_peers_can_be_decomposed_and_recomposed() {
        deadline(async {
            ...
            let mut left = Duplex::new(sender, receiver);
    ...
    async fn stores_forward_between_non_overlapping_peer_sessions() {
        deadline(async {
            let (mut a_store, mut b_store) = store_and_forward(MemConfig::default());
            {
                ...
                let (sender, receiver) = a.into_parts();
                assert_eq!(sender.peer(), receiver.peer());
            }
            {
